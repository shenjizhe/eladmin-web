    ...
    <div class="head-container">
      ...
      <el-table ref="table" v-loading="crud.loading" :data="crud.data" size="small" style="width: 100%;" @selection-change="crud.selectionChangeHandler">
        ...
        <el-table-column prop="pk" label="是否主键">
          <template slot-scope="scope">
            {{ dict.label.show_status[scope.row.pk] }}
          </template>
        </el-table-column>
        <el-table-column prop="type" label="数据类型">
          <template slot-scope="scope">
            {{ dict.label.db_types[scope.row.type] }}
          </template>
        </el-table-column>
        <el-table-column v-if="checkPer(['admin','entityField:edit','entityField:del'])" label="操作" width="150px" align="center">
          <template slot-scope="scope">
            <udOperation
              :data="scope.row"
...
const defaultForm = { id: null, entityId: null, name: null, comment: null, show: null, pk: null, type: null }
export default {
  name: 'EntityField',
  components: { pagination, crudOperation, rrOperation, udOperation },
  mixins: [presenter(), header(), form(defaultForm), crud()],
  dicts: ['show_status', 'db_types'],
  cruds() {
    return CRUD({ title: '实体属性', url: 'api/entityField', idField: 'id', sort: 'id,desc', crudMethod: { ...crudEntityField }})
  },
  data() {
    return {
      permission: {
